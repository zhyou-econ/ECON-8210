...
function compute_L_opt(τ, w_curr, r_curr, K, I)
    ...
    if discriminant < 0
        ...
        if L_opt <= 0
            return NaN  # Non-positive labor is not feasible
        else
            return L_opt
        end
    end
                ...
                for I_idx = 1:num_I
                    ...
                    for K_idx = 1:num_K
                        ...
                        function obj_I(I_vec)
                            ...
                            if isnan(L_opt)
                                return 1e10  # Penalize infeasible choices
                            end
                            # Compute consumption
                            ...
                            V_future = reshape(V_future, num_τ, num_z)
                            V_expect = sum(π_joint .* V_future)
                            # Current utility
                            utility = log(C) - 0.5 * L_opt^2 + β * V_expect
