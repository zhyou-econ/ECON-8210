...
function solve_household!(params::ModelParams, w::Array{Float64,4}, r::Array{Float64,4}, V_guess::Array{Float64,4}, max_iter, tol, mode)
    ...
    for iter = 1:max_iter
        ...
        if mode==3
            # Redefine K_grid for the stochastic case
            K_grid = zeros(num_K)
            K_dist = Truncated(Normal(K_ss, 0.1 * K_ss), 0.75 * K_ss, 1.25 * K_ss)
            for i in 1:num_K
                K_grid[i] = rand(K_dist)
            end
        end
        K_grid = sort(K_grid)
        params.K_grid = K_grid
        ...
        @threads for τ_z_idx in 1:(num_τ * num_z)
            ...
            for I_idx = 1:num_I
                ...
                for K_idx = 1:num_K
                    ...
                    if mode == 1 || mode == 3 #Max every period
                    ...
                    V_error = max(V_error, abs(V_new[τ_idx, z_idx, I_idx, K_idx] - V_interp[τ_idx, z_idx](I_minus, K)))
                end
            end
        end
    ...
    return V, L_policy, I_policy, K_policy, K_grid
end
